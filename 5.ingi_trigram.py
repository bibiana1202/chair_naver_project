...
def df_word_good(list1_good):
    ...
    df_word_good.to_csv(path_or_buf=address_review+file_name+'_good_trigram'+'.csv',encoding = 'utf-8-sig')
    return df_word_good

def df_valuecounts_good(df_word_good):
    df_valuecounts_good=df_word_good['word'].value_counts()
    df_valuecounts_good.to_csv(path_or_buf=address_review+file_name+'_good_valuecount_trigram'+'.csv',encoding = 'utf-8-sig')

def my_tokenizer_with_trigram(list_tokens):
    list_d=[]
    for i in tqdm(range(len(list_tokens)-2),desc='trigram'):
        if(list_tokens[i]=='' or list_tokens[i+1]=='' or list_tokens[i+2]==''):
            continue
        list_d.append(' '.join(list_tokens[i:i+3]))
    return list_d
##############################################################################################################################################################################################################################################################################################################################################################################################################################################################################
##############################################################################################################################################################################################################################################################################################################################################################################################################################################################################
...
list_good = my_tokenizer_with_trigram(list_good)
