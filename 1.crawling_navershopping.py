...
df_products.to_csv('df_ingi_product.csv',encoding='utf-8-sig')
    ...
    if "smartstore" in list_product["url"]:
        ...
        while True:
            ...
            try:
                ...
                print('except',res.text)
                break
    ####### 2) 서치 네이버 쇼핑 제품 리뷰 가져오기
    else:
        # count가 리뷰하나마다 1씩 증가하고 total_count 가 되면 멈추는 while 문
        count = 0
        page_num = 1
...
df_list_of_reviews.to_csv("df_ingi_review.csv", encoding='utf-8-sig')
