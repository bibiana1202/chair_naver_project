...
def df_word_good(list1_good):
    ...
    df_word_good.to_csv(path_or_buf=address_review+file_name+'_good_B'+'.csv',encoding = 'utf-8-sig')
    return df_word_good

def df_valuecounts_good(df_word_good):
    df_valuecounts_good=df_word_good['word'].value_counts()
    df_valuecounts_good.to_csv(path_or_buf=address_review+file_name+'_good_valuecount_B'+'.csv',encoding = 'utf-8-sig')
