...
def df_word_bad(list1_bad):
    ...
    df_word_bad.to_csv(path_or_buf=address_review+file_name+'_bad_B'+'.csv',encoding = "cp949")
    return df_word_bad


def df_valuecounts_bad(df_word_bad):
    df_valuecounts_bad=df_word_bad['word'].value_counts()
    df_valuecounts_bad.to_csv(path_or_buf=address_review+file_name+'_bad_valuecount_B'+'.csv',encoding = "cp949")
